        ...
        num_of_agents(instance.getDefaultNumberOfAgents())
{
    ...
    {
        if (sipp){
        	std::cout<<"***************start SIPP"<<std::endl;
            search_engines[i] = new SIPP(instance, i);
        	std::cout<<"***************end SIPP"<<std::endl;
...
bool PBS::hasConflicts(int a1, int a2) const
{
	...
	{
		int loc1 = paths[a1]->at(timestep).node.location;
		int loc2 = paths[a2]->at(timestep).node.location;
		if (loc1 == loc2 or (timestep < min_path_length - 1 and loc1 == paths[a2]->at(timestep + 1).node.location
                             and loc2 == paths[a1]->at(timestep + 1).node.location)) // vertex or edge conflict
		{
            return true;
		}
	}
	if (paths[a1]->size() != paths[a2]->size())
	{
		...
		int loc1 = paths[a1_]->back().node.location;
		for (int timestep = min_path_length; timestep < (int)paths[a2_]->size(); timestep++)
		{
			int loc2 = paths[a2_]->at(timestep).node.location;
			if (loc1 == loc2)
			{
...
void PBS::printPaths() const
{
	for (int i = 0; i < num_of_agents; i++)
	{
		...
		for (const auto & t : *paths[i])
			cout << t.node.location << "," <<t.node.direction << "->";
		cout << endl;
	}
}
...
void PBS::savePaths(const string &fileName) const
{
    ...
    for (int i = 0; i < num_of_agents; i++)
    {
        output << "Agent " << i << ": ";
        for (const auto & t : *paths[i])
            output << "(" << search_engines[0]->instance.getRowCoordinate(t.node.location)
                   << "," << search_engines[0]->instance.getColCoordinate(t.node.location)
                   << "," << t.node.direction << ")->";
        output << endl;
    }
    output.close();
		...
		for (int a2 = a1 + 1; a2 < num_of_agents; a2++)
		{
			...
			for (size_t timestep = 0; timestep < min_path_length; timestep++)
			{
				...
				if (loc1 == loc2)
				{
					...
					&& loc1 == paths[a2]->at(timestep + 1).node.location
					&& loc2 == paths[a1]->at(timestep + 1).node.location)
				{
...
inline int PBS::getAgentLocation(int agent_id, size_t timestep) const
{
	...
	return paths[agent_id]->at(t).node.location;
}
