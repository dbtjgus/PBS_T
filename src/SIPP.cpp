...
void SIPP::updatePath(const LLNode* goal, vector<PathEntry> &path)
{
    ...
    while (curr->parent != nullptr) // non-root node
    {
        ...
        {
            path[t].node = prev->node; // wait at prev location
            t++;
        }
        path[curr->timestep].node = curr->node; // move to curr location
        curr = prev;
    }
    assert(curr->timestep == 0);
    path[0].node = curr->node;
...
Path SIPP::findOptimalPath(const set<int>& higher_agents, const vector<Path*>& paths, int agent)
{
    ...
    auto start = new SIPPNode(Node(start_location,0,0), 0, max(my_heuristic[start_location], holding_time), nullptr, 0,
    ...
    while (!open_list.empty())
    {
        ...
        if (curr->node.location == goal_location && // arrive at the goal location
        ...
        for (Node next_node : instance.getNeighbors(curr->node)) // move to neighboring locations
        {   
            if(next_node.location != curr->node.location){
                for (auto & i : reservation_table.get_safe_intervals(
                        curr->node.location, next_node.location, curr->timestep + 1, curr->high_expansion + 1))
                {
                    ...
                    int next_h_val = max(my_heuristic[next_node.location], curr->getFVal() - next_g_val);  // path max
                    ...
                    auto next = new SIPPNode(next_node, next_g_val, next_h_val, curr, next_timestep,
                ...
                if(curr->timestep+1 <= curr->high_expansion){
                    auto next = new SIPPNode(next_node, curr->g_val+1, curr->h_val, curr, curr->timestep+1, 
                        curr->high_generation, curr->high_expansion, curr->collision_v, curr->num_of_conflicts);
                    if (dominanceCheck(next))
                        pushNodeToOpen(next);
                    else
                        delete next;
        ...
        if (curr->high_expansion == curr->high_generation and
            reservation_table.find_safe_interval(interval, curr->node.location, curr->high_expansion) and
            get<0>(interval) + curr->h_val <= reservation_table.constraint_table.length_max)
        {
            ...
            auto next_collisions = curr->num_of_conflicts +
                                   ...
                                   + (int)get<2>(interval);
            auto next = new SIPPNode(curr->node, next_timestep, next_h_val, curr, next_timestep,
                                     get<1>(interval), get<1>(interval), get<2>(interval), next_collisions);
            if (curr->node.location == goal_location)
