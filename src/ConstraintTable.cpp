...
{
    int prev_location = path.front().node.location;
    int prev_timestep = 0;
    for (int timestep = 0; timestep < (int) path.size(); timestep++)
    {
        auto curr_location = path[timestep].node.location;
    ...
    insert2CT(path.back().node.location, (int) path.size() - 1, MAX_TIMESTEP);
}
...
void ConstraintTable::insert2CAT(const Path& path)
{
    ...
    assert(cat_goals[path.back().node.location] == MAX_TIMESTEP);
    cat_goals[path.back().node.location] = path.size() - 1;
    for (auto timestep = (int)path.size() - 1; timestep >= 0; timestep--)
    {
        int loc = path[timestep].node.location;
