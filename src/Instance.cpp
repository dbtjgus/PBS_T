...
list<Node> Instance::getNeighbors(Node curr) const
{
	list<Node> neighbors;
	//int candidates[4] = {curr + 1, curr - 1, curr + num_of_cols, curr - num_of_cols};
	Node candidates[4];
	for(int i=0;i<4;i++){
		if(curr.direction != i*90)
			candidates[i] = Node(curr.location, 0,i*90);
		else if(curr.direction==0)
			candidates[i] = Node(curr.location+1, 0,curr.direction);
		else if(curr.direction==90)
			candidates[i] = Node(curr.location+num_of_cols, 0, curr.direction);
		else if(curr.direction==180)
			candidates[i] = Node(curr.location-1, 0, curr.direction);
		else if(curr.direction==270)
			candidates[i] = Node(curr.location-num_of_cols, 0, curr.direction);
	}
