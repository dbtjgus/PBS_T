...
Path SpaceTimeAStar::findOptimalPath(const set<int>& higher_agents, const vector<Path*>& paths, int agent)
{
    ...
    auto start = new AStarNode(Node(start_location,0,0), 0, max(holding_time, my_heuristic[start_location]), nullptr, 0, 0);
    ...
    while (!open_list.empty())
    {
        ...
        auto next_nodes = instance.getNeighbors(curr->node);
        next_nodes.emplace_back(curr->node);
        for (Node next_node : next_nodes)
        {
            ...
            { // now everything is static, so switch to space A* where we always use the same timestep
                if (next_node.location == curr->node.location && next_node.direction == curr->node.direction)
                {
                    continue;
                }
                next_timestep--;
            ...
            if (constraint_table.constrained(next_node.location, next_timestep) ||
                constraint_table.constrained(curr->node.location, next_node.location, next_timestep))
                continue;

            // compute cost to next_id via curr node
            int next_g_val = curr->g_val + 1;
            int next_h_val = max(holding_time - next_g_val, my_heuristic[next_node.location]);
            if (next_g_val + next_h_val > constraint_table.length_max)
                continue;
            int next_internal_conflicts = curr->num_of_conflicts +
                                          constraint_table.getNumOfConflictsForStep(curr->node.location, next_node.location, next_timestep);

            // generate (maybe temporary) node
            auto next = new AStarNode(next_node, next_g_val, next_h_val,
