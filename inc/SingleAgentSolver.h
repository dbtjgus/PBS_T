...
public:
	...
	LLNode() : node(Node()), g_val(0), h_val(0), parent(nullptr), timestep(0), num_of_conflicts(0), in_openlist(false), wait_at_goal(false) {}

	LLNode(Node node, int g_val, int h_val, LLNode* parent, int timestep, int num_of_conflicts = 0, bool in_openlist = false) :
		node(node), g_val(g_val), h_val(h_val), parent(parent), timestep(timestep),
...
public:
	...
	list<Node> getNextLocations(Node curr) const; // including itself and its neighbors
	list<int> getNeighbors(int curr) const { return instance.getNeighbors(curr); }
	list<Node> getNeighbors(Node curr) const { return instance.getNeighbors(curr); }
