...
public:
    ...
    bool collision_v;
    SIPPNode() : LLNode() {}
    SIPPNode(Node node, int g_val, int h_val, SIPPNode* parent, int timestep, int high_generation, int high_expansion,
             bool collision_v, int num_of_conflicts) :
            LLNode(node, g_val, h_val, parent, timestep, num_of_conflicts), high_generation(high_generation),
    ...
    struct NodeHasher
    {
        ...
        {
            size_t seed = 0;
            boost::hash_combine(seed, n->node.location);
            boost::hash_combine(seed, n->node.direction);
    ...
    struct eqnode
    {
        ...
        {
            return (n1 == n2) ||
                   (n1 && n2 && n1->node.location == n2->node.location &&
