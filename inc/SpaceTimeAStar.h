...
public:
	...
	AStarNode(Node node, int g_val, int h_val, LLNode* parent, int timestep, int num_of_conflicts = 0, bool in_openlist = false) :
		LLNode(node, g_val, h_val, parent, timestep, num_of_conflicts, in_openlist) {}
	...
	struct NodeHasher
	{
		...
		{
			size_t loc_hash = std::hash<int>()(n->node.location);
	...
	struct eqnode
	{
		...
		{
			return (s1 == s2) || (s1 && s2 &&
                        s1->node.location == s2->node.location &&
                        s1->node.direction == s2->node.direction &&
